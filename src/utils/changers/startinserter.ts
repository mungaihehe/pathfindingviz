...
export class StartInserter extends TileChanger {
  ...
  change(grid: Grid): Grid {
    ...
    if (this.coordinate.isEqualTo(currentStartCoordinate)) return grid;
    if (this.coordinate.isEqualTo(currentStopCoordinate)) {
    ...
    return produce(grid, (draft) => {
      currentStartCoordinate.isValidInGrid(draft) &&
        draft.setTile(new TileData(TileType.normal), currentStartCoordinate);
      draft.setTile(new TileData(TileType.start), this.coordinate);
    });
  }
}
